...
const protectedUserRoutes = ["/user", "/user/invoices", "/user/bookings"];
...
export async function middleware(request: NextRequest) {
    ...
    const AdminCookie = (await cookies()).get("AdminSession")?.value;
    ...
    const UserCookie = (await cookies()).get("UserSession")?.value;
